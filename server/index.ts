...
async function initStripe() {
  ...
  try {
    ...
    // FIX: Soportar tanto Render como Replit
    ...
    // Asegurarse de que tiene https://
    const fullWebhookUrl = webhookBaseUrl.startsWith('http') 
      ? webhookBaseUrl 
      : `https://${webhookBaseUrl}`;
    
    try {
      const result = await stripeSync.findOrCreateManagedWebhook(
        `${fullWebhookUrl}/api/stripe/webhook`
      );
